...
export class PostsController {
  ...
  findUserPosts(@PaginationParams() pagination: Pagination, @Query('username') username: string) {
    if (!username) {
      throw new BadRequestException('Имя пользователя обязательно для поиска постов')
    }
  ...
  @Patch(':slug')
  async update(@Param('slug') slug: string, @Body() updatePostDto: UpdatePostDto, @Req() request: Request) {
    const userId = +request?.user?.id
    return this.postsService.update(userId, slug, updatePostDto)
  }

  @Delete(':slug')
  remove(@Param('slug') slug: string, @Req() request: Request) {
    const userId = +request?.user?.id
